...
namespace ClashRoyale.Protocol.Commands.Client
{
    public class LogicChallengeCommand : LogicCommand
    {
        ...
        {
            Logger.Log($"Gamemode: {GameMode}", null);

            if (GameMode == 0) // 0 = normal friendly battle
            {
            ...
            else if (GameMode == 23) // 23 = clan 2v2 battle (TODO)
            {
                var home = Device.Player.Home;
                var alliance = await Resources.Alliances.GetAllianceAsync(home.AllianceInfo.Id);
                ...
                {
                    EstimatedDuration = 300 // seconds (300 = 5 min)
                }.SendAsync();
            }
            else
            {
