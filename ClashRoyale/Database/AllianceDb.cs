...
#pragma warning restore 618
        ...
        public static async Task<List<Alliance>> GetLocalAllianceRankingAsync(string region)
        {
            #region GetLocal
            ...
            try
            {
                using (var connection = new MySqlConnection(_connectionString))
                {
                    ...
                    using (var cmd =
                        new MySqlCommand(
                            $"SELECT * FROM {Name} WHERE Region = '{region}' ORDER BY `Trophies` DESC LIMIT 200",
                            connection))
                    {
                        var reader = await cmd.ExecuteReaderAsync();
