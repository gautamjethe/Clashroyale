    ...
    public class PlayerDb
    {
        ...
        public static async Task<Player> CreateAsync()
        {
            ...
            try
            {
                ...
                using (var cmd =
                    new MySqlCommand(
                        $"INSERT INTO {Name} (`Id`, `Trophies`, `Language`, `FacebookId`, `Home`, `Sessions`) VALUES ({id + 1}, {(long) player.Home.Arena.Trophies}, @language, @fb, @home, @sessions)")
                )
                {
...
#pragma warning restore 618
        ...
        public static async Task SaveAsync(Player player)
        {
            ...
            try
            {
                using (var cmd =
                    new MySqlCommand(
                        $"UPDATE {Name} SET `Trophies`='{(long) player.Home.Arena.Trophies}', `Language`=@language, `FacebookId`=@fb, `Home`=@home, `Sessions`=@sessions WHERE Id = '{player.Home.Id}'")
                )
                {
#pragma warning disable 618
                    cmd.Parameters?.AddWithValue("@language", player.Home.PreferredDeviceLanguage);
